...
function rollDice() {
    ...
    Swal.fire({
        ...
        iconHtml: '<img src="https://media.tenor.com/WiTP5aZyPLUAAAAi/dice-roll-dice.gif" width=50px>',
        iconColor: "#8CC63E",
...
function chromaticChaos(){
    Swal.fire({
        ...
        customClass: {
    popup: 'chromatic-chaos',
    backdrop: '.chromatic-chaos-backdrop',
...
function symphonyOfSwiftness(){
    Swal.fire({
      ...
      customClass: {
            ...
            title: 'symphony-swiftness-title',
            htmlContainer: 'symphony-swiftness-content'
  }
    }).then((result) => {
      if (result.isConfirmed) {
          var trialMax = document.getElementById("trials-progress").max;
...
function initializeProgressBars(valuesDict) {
    ...
    const progressBarMappings = {
        "ptsPerTrial": "trials-progress",
        "playerStartingHealth": "player-health",
        "numTrials": "adventure-progress",
    };
...
function saveGame() {
    ...
    savedGameArea.classList.add("text-xs");
}
